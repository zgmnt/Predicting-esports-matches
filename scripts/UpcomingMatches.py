...
matchTeam = soup.find_all(class_="matchTeam")

for elem in matchTeam:
    indexBegin = str(elem).find("text-ellipsis")
    indexEnd = str(elem).find("</div>")
    print(str(elem)[indexBegin+len('text-ellipsis">'):].replace("</div>", "").replace("\n", ""))
...
teamNames = soupMatch.find_all(class_="text-ellipsis")
teamsInMatchTable = []
teamsInMatchTuples = []
